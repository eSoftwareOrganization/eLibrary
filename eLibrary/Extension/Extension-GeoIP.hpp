...
namespace eLibrary::Extension::GeoIP {
    ...
    private:
        ...
        static String doResolve(::MMDB_lookup_result_s DatabaseResult, Ts&& ...DatabaseParameter) {
            if (!DatabaseResult.found_entry) throw IO::IOException(u"MaxMindDatabase::doResolve(MMDB_lookup_result_s) DatabaseResult.found_entry"_S);
            ...
            if (!DatabaseEntryData.has_data || DatabaseEntryData.type != MMDB_DATA_TYPE_UTF8_STRING) throw IO::IOException(u"MaxMindDatabase::doResolve(MMDB_lookup_result_s) MMDB_get_value"_S);
    ...
    public:
        MaxMindDatabase(const String &DatabaseName, uint32_t DatabaseMode) {
            if (::MMDB_open(DatabaseName.toU8String().c_str(), DatabaseMode, &DatabaseObject) != MMDB_SUCCESS)
                throw IO::IOException(u"MaxMindDatabase::doOpen(const String&) MMDB_open"_S);
        }

...
namespace eLibrary::Multimedia {
    ...
    public:
        MediaCodec(MediaCodec &&CodecSource) noexcept : CodecObject(CodecSource.CodecObject) {
        ...
        static MediaCodec doFindDecoder(enum AVCodecID CodecID) {
            ...
            if (!CodecObject) throw MediaException(String(u"MediaCodec::doFindDecoder(AVCodecID) avcodec_find_decoder"));
            return {CodecObject};
        }

        static MediaCodec doFindEncoder(enum AVCodecID CodecID) {
            const AVCodec *CodecObject = avcodec_find_encoder(CodecID);
            if (!CodecObject) throw MediaException(String(u"MediaCodec::doFindEncoder(AVCodecID) avcodec_find_encoder"));
            return {CodecObject};
        }
    ...
    private:
        ...
        friend class MediaCodecContext;
    public:
        MediaFrame(MediaFrame &&FrameSource) noexcept : FrameObject(FrameSource.FrameObject) {
    ...
    public:
        MediaPacket(MediaPacket &&PacketSource) noexcept : PacketObject(PacketSource.PacketObject) {
    ...
    public:
        MediaSWRContext(MediaSWRContext &&ContextSource) noexcept : ContextObject(ContextSource.ContextObject) {
        ...
        static MediaSWRContext doAllocate() {
            SwrContext *ContextObject = swr_alloc();
            if (!ContextObject) throw MediaException(String(u"MediaSWRContext::doAllocate() swr_alloc"));
    ...
    public:
        MediaCodecContext(MediaCodecContext &&ContextSource) noexcept : ContextObject(ContextSource.ContextObject) {
            ContextSource.ContextObject = nullptr;
        }
        ...
        static MediaCodecContext doAllocate() {
            ...
            if (!CodecContextObject) throw MediaException(String(u"MediaCodecContext::doAllocate() avcodec_alloc_context3"));
        ...
        static MediaCodecContext doAllocate(const MediaCodec &ContextCodecSource) {
            ...
            if (!CodecContextObject) throw MediaException(String(u"MediaCodecContext::doAllocate(const MediaCodec&) avcodec_alloc_context3"));
            return {CodecContextObject};
        }
    ...
    public:
        MediaFormatContext(MediaFormatContext &&ContextSource) noexcept : ContextObject(ContextSource.ContextObject) {
            ContextSource.ContextObject = nullptr;
        }
        ...
        int doFindBestStream(enum AVMediaType StreamType) const {
            ...
            if (MediaStreamIndex < 0) throw MediaException(String(u"MediaFormatContext::doFindBestStream(AVMediaType) av_find_best_stream"));
        ...
        static MediaFormatContext doAllocate() {
            ...
            if (!ContextObject) throw MediaException(String(u"MediaFormatContext::doAllocate() avformat_alloc_context"));
            return {ContextObject};
        }

        static MediaFormatContext doAllocateOutput(const String &MediaPath) {
            AVFormatContext *ContextObject = nullptr;
            if (avformat_alloc_output_context2(&ContextObject, nullptr, nullptr, MediaPath.toU8String().c_str()) < 0) throw MediaException(String(u"MediaFormatContext::doAllocateOutput(const String&) avformat_alloc_output_context2"));
            return {ContextObject};
        }
        ...
        void setOutputFormat(const AVOutputFormat *FormatSource) {
            if (!FormatSource) throw MediaException(String(u"MediaFormatContext::setOutputFormat(const AVOutputFormat*) FormatSource"));
